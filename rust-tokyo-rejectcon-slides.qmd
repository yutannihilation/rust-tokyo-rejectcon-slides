...
- 私が開発している savvy については今日はほぼ話しません。興味がある方はこちらをどうぞ。
...
* たとえば、R だと NA は伝播するが
  ...
  #> [1] NA
  ```
  Rust 側で `NA` を考慮せずそのまま足すと、 `i32::MIN + 1`（= `-2147483647`）になってしまう。
* `R_IsNA()` など欠損値を判定する API があるので、いちいちそれを使うようにする。
