...
- 私が開発している savvy については今日は話しません。興味がある方はこちらをどうぞ。
...
- C のヘッダファイルを元に bindgen などで Rust binding を生成する
（よくある感じ）

## R から Rust を使う

1. `extern "C"` で関数を書き、それに対応する C のヘッダファイルも用意する
...
* 今のところ、「Python もあるなら R でもつくるか」という程度のものがほとんど
  * まあこれも、C++ のライブラリだとビルドの難易度が高くてできなかったケースは多く、Rust のおかげではある
* キラーアプリ的なものが欲しいところ
...
* （他の言語と同じく）R には R 固有の仕組みがあって、Rust を使うときにはいくつかのポイントで工夫が必要
* 加えて、R に固有の事情として、公式のパッケージレジストリが使いづらいのでいろいろ苦労する
